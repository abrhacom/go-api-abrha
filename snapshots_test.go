...
func TestSnapshots_List(t *testing.T) {
	...
	mux.HandleFunc("/api/public/v1/snapshots", func(w http.ResponseWriter, r *http.Request) {
...
func TestSnapshots_ListVm(t *testing.T) {
	setup()
	defer teardown()
	...
	ctx := context.Background()
	snapshots, _, err := client.Snapshots.ListVm(ctx, nil)
	if err != nil {
		t.Errorf("Snapshots.ListVm returned error: %v", err)
	}

	expected := []Snapshot{{ID: "1"}, {ID: "2", SizeGigaBytes: 4.84}}
	if !reflect.DeepEqual(snapshots, expected) {
		t.Errorf("Snapshots.ListVm returned %+v, expected %+v", snapshots, expected)
	}
}
...
func TestSnapshots_GetSnapshotByID(t *testing.T) {
	...
	mux.HandleFunc("/api/public/v1/snapshots/12345", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, http.MethodGet)
...
func TestSnapshot_String(t *testing.T) {
	...
	expected := `go_api_abrha.Snapshot{ID:"1", Name:"Snapsh176ot", ResourceID:"0", ResourceType:"vm", Regions:["one"], MinDiskSize:20, SizeGigaBytes:4.84, Created:"2013-11-27T09:24:55Z", Tags:["one" "two"]}`
