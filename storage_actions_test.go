...
func TestStoragesActions_Attach(t *testing.T) {
	...
	attachRequest := &ActionRequest{
		"type":  "attach",
		"vm_id": vmID, // encoding/json decodes numbers as floats
	}

	mux.HandleFunc("/api/public/v1/volumes/"+volumeID+"/actions", func(w http.ResponseWriter, r *http.Request) {
		v := new(ActionRequest)
	...
	_, _, err := client.StorageActions.Attach(ctx, volumeID, vmID)
	if err != nil {
		t.Errorf("StoragesActions.Attach returned error: %v", err)
	}
}

func TestStoragesActions_DetachByVmID(t *testing.T) {
	setup()
	defer teardown()
	volumeID := "98d414c6-295e-4e3a-ac58-eb9456c1e1d1"
	vmID := "123456"

	detachByVmIDRequest := &ActionRequest{
		"type":  "detach",
		"vm_id": vmID, // encoding/json decodes numbers as floats
	}
	...
	mux.HandleFunc("/api/public/v1/volumes/"+volumeID+"/actions", func(w http.ResponseWriter, r *http.Request) {
		...
		testMethod(t, r, http.MethodPost)
		if !reflect.DeepEqual(v, detachByVmIDRequest) {
			t.Errorf("want=%#v", detachByVmIDRequest)
			t.Errorf("got=%#v", v)
		}
		fmt.Fprintf(w, `{"action":{"status":"in-progress"}}`)
	})

	_, _, err := client.StorageActions.DetachByVmID(ctx, volumeID, vmID)
	if err != nil {
		t.Errorf("StoragesActions.DetachByVmID returned error: %v", err)
	}
}
...
func TestStorageActions_Get(t *testing.T) {
	...
	mux.HandleFunc("/api/public/v1/volumes/"+volumeID+"/actions/456", func(w http.ResponseWriter, r *http.Request) {
