...
func TestLoadBalancers_Get(t *testing.T) {
	...
	path := "/api/public/v1/load_balancers"
...
func TestLoadBalancers_AddVms(t *testing.T) {
	setup()
	defer teardown()

	vmIdsRequest := &vmIDsRequest{
		IDs: []string{"42", "44"},
	}

	lbID := "37e6be88-01ec-4ec7-9bc6-a514d4719057"
	path := fmt.Sprintf("/api/public/v1/load_balancers/%s/vms", lbID)
	mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		v := new(vmIDsRequest)
		err := json.NewDecoder(r.Body).Decode(v)
		if err != nil {
		...
		testMethod(t, r, http.MethodPost)
		assert.Equal(t, vmIdsRequest, v)

		fmt.Fprint(w, nil)
	})

	_, err := client.LoadBalancers.AddVms(ctx, lbID, vmIdsRequest.IDs...)

	assert.NoError(t, err)
}

func TestLoadBalancers_RemoveVms(t *testing.T) {
	setup()
	defer teardown()
	...
	_, err := client.LoadBalancers.RemoveVms(ctx, lbID, vmIdsRequest.IDs...)
...
func TestLoadBalancers_AddForwardingRules(t *testing.T) {
	...
	lbID := "37e6be88-01ec-4ec7-9bc6-a514d4719057"
	path := fmt.Sprintf("/api/public/v1/load_balancers/%s/forwarding_rules", lbID)
	mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
...
func TestLoadBalancers_AsRequest(t *testing.T) {
	...
	lb.VmIDs = append(lb.VmIDs, "13579")
	...
	assert.Equal(t, []string{"12345", "54321"}, r.VmIDs)
