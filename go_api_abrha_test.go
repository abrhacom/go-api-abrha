...
func TestNewRequest(t *testing.T) {
	...
	inBody, outBody := &VmCreateRequest{Name: "l"},
...
func TestNewRequest_withUserData(t *testing.T) {
	...
	inURL, outURL := "foo", defaultBaseURL+"foo"
	inBody, outBody := &VmCreateRequest{Name: "l", UserData: "u"},
		`{"name":"l","region":"","size":"","image":0,`+
			`"ssh_keys":null,"backups":false,"ipv6":false,`+
...
func TestNewRequest_withVmAgent(t *testing.T) {
	c := NewClient(nil)

	boolVal := true
	inURL, outURL := "foo", defaultBaseURL+"foo"
	inBody, outBody := &VmCreateRequest{Name: "l", WithVmAgent: &boolVal},
		`{"name":"l","region":"","size":"","image":0,`+
			`"ssh_keys":null,"backups":false,"ipv6":false,`+
			`"private_networking":false,"monitoring":false,"tags":null,"with_vm_agent":true}`+"\n"
	req, _ := c.NewRequest(ctx, http.MethodPost, inURL, inBody)
...
func TestWithRetryAndBackoffsForResourceMethods(t *testing.T) {
	...
	expectingErr := fmt.Sprintf("GET %s/api/public/v1/account: 500 broken; giving up after 4 attempt(s)", url)
	_, resp, err := client.Account.Get(context.Background())
	if err == nil || (err.Error() != expectingErr) {
		t.Fatalf("expected giving up error, got: %s", err.Error())
	}
	if _, ok := err.(*ErrorResponse); !ok {
		t.Fatalf("expected error to be *goApiAbrha.ErrorResponse, got: %#v", err)
	}

	// Ensure that the *Response is properly populated
	if resp == nil {
		t.Fatal("expected non-nil *goApiAbrha.Response")
	}
