...
const (
	...
	loadBalancersBasePath = "/api/public/v1/load_balancers"
)
...
func (l LoadBalancer) AsRequest() *LoadBalancerRequest {
	r := LoadBalancerRequest{
		...
		VmIDs:                        append([]string(nil), l.VmIDs...),
		Tag:                          l.Tag,
...
type vmIDsRequest struct {
	IDs []string `json:"vm_ids,omitempty"`
}

func (l vmIDsRequest) String() string {
	return Stringify(l)
}
...
func (l *LoadBalancersServiceOp) AddVms(ctx context.Context, lbID string, vmIDs ...string) (*Response, error) {
	path := fmt.Sprintf("%s/%s/%s", loadBalancersBasePath, lbID, vmsPath)

	req, err := l.client.NewRequest(ctx, http.MethodPost, path, &vmIDsRequest{IDs: vmIDs})
	if err != nil {
		return nil, err
	}
...
func (l *LoadBalancersServiceOp) RemoveVms(ctx context.Context, lbID string, vmIDs ...string) (*Response, error) {
	...
	req, err := l.client.NewRequest(ctx, http.MethodDelete, path, &vmIDsRequest{IDs: vmIDs})
	if err != nil {
		return nil, err
	}
